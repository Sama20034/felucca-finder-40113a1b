...
export const CartDrawer = () => {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent side={isRTL ? "left" : "right"} className="w-full sm:max-w-lg flex flex-col h-full">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle>{isRTL ? "سلة التسوق" : "Shopping Cart"}</SheetTitle>
          <SheetDescription>
            {totalItems === 0 
              ? (isRTL ? "سلتك فارغة" : "Your cart is empty")
              : (isRTL ? `${totalItems} منتج في سلتك` : `${totalItems} item${totalItems !== 1 ? 's' : ''} in your cart`)
            }
          </SheetDescription>
        </SheetHeader>
          ...
          {items.length === 0 ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">{isRTL ? "سلتك فارغة" : "Your cart is empty"}</p>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t bg-background">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-primary">
                    {items[0]?.price.currencyCode || 'EGP'} {totalPrice.toFixed(2)}
                  </span>
                </div>
                <Button 
